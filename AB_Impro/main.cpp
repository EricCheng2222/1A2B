...
bool isAnswer[10000];
double possibility[10000];
vector<char*> Test;
vector<char*> Result;

void printHighestProb(){
    double tmp[10000];
    ...
    for (int i=9999; i>=9899; i--) {
        for (int j=0; j<10000; j++) {
            if (possibility[j] == tmp[i] && used[j]==false) {
                printf("%d ", j);
                used[j] = true;
                break;
            }
...
void calculatePossibility(){
    for (int i=0; i<10000; i++) {
        ...
        if (isAnswer[i]==true) {
            int sep[4];
            int tmp = i;
            sep[3] = tmp%10; tmp = tmp/10;
            sep[2] = tmp%10; tmp = tmp/10;
            sep[1] = tmp%10; tmp = tmp/10;
            sep[0] = tmp;
            
            possibility[i] = likelihood[sep[3]] + likelihood[sep[2]] + likelihood[sep[1]] + likelihood[sep[0]];
        }
    }
}
